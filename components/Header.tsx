...
export default function Header() {
    ...
    const [isMapOnline, setIsMapOnline] = useState(true);
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
    const adminDropdownRef = useRef<HTMLDivElement>(null);
    const mapDropdownRef = useRef<HTMLDivElement>(null);

    // Health check for Map Service deployment
    useEffect(() => {
        ...
        fetch(serviceUrl, { method: "HEAD", mode: "no-cors", signal: controller.signal })
            .then(() => setIsMapOnline(true))
            .catch(() => setIsMapOnline(false))
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16">
                    ...
                    <nav className="hidden md:flex space-x-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                        ...
                        {/* Civic Map Dropdown (only rendered if map deployment is online/reachable) */}
                        {isMapOnline && (
