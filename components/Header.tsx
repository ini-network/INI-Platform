                    ...
                    <nav className="hidden md:flex space-x-1 bg-slate-50 p-1 rounded-xl border border-slate-100">
                        ...
                        <div className="relative flex items-center" ref={mapDropdownRef}>
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-1.5 focus:outline-none ${["/map", "/news", "/reports"].some(p => pathname.startsWith(p))
                                        ? "bg-white text-blue-700 shadow-sm"
                                        : "text-slate-500 hover:text-slate-800 hover:bg-slate-100"
                                    }`}
                            >
                            ...
                            {isMapOpen && (
                                <div className="absolute left-0 mt-2 top-full w-48 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 p-1.5 space-y-1">
                                    <Link
                                        ...
                                        className={`block px-3.5 py-2.5 text-xs font-bold rounded-xl transition-colors ${pathname.startsWith("/map") ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}
                                    >
                                        🗺️ Interactive Map
                                    </Link>
                                    <Link
                                        href="/news"
                                        onClick={() => setIsMapOpen(false)}
                                        className={`block px-3.5 py-2.5 text-xs font-bold rounded-xl transition-colors ${pathname.startsWith("/news") ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}
                                    >
                                        📰 Civic News
                                    </Link>
                                    <Link
                                        href="/reports"
                                        onClick={() => setIsMapOpen(false)}
                                        className={`block px-3.5 py-2.5 text-xs font-bold rounded-xl transition-colors ${pathname.startsWith("/reports") ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}
                                    >
                                        🗣️ Civic Stories
                                    </Link>
                                </div>
                ...
                <div className="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-md shadow-xl animate-in slide-in-from-top duration-200 absolute w-full top-full left-0">
                    <div className="px-4 pt-3 pb-6 space-y-1.5 max-h-[80vh] overflow-y-auto">
                        ...
                        <div className="pt-4 mt-4 border-t border-slate-100 space-y-1.5">
                            ...
                            <Link href="/map" onClick={() => setIsMobileMenuOpen(false)} className={`block px-4 py-3 rounded-xl text-sm font-bold transition-all ${pathname.startsWith("/map") ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
                                Interactive Map
                            </Link>
                            <Link href="/news" onClick={() => setIsMobileMenuOpen(false)} className={`block px-4 py-3 rounded-xl text-sm font-bold transition-all ${pathname.startsWith("/news") ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
                                Civic News
                            </Link>
                            <Link href="/reports" onClick={() => setIsMobileMenuOpen(false)} className={`block px-4 py-3 rounded-xl text-sm font-bold transition-all ${pathname.startsWith("/reports") ? "bg-blue-50 text-blue-700 shadow-sm" : "text-slate-600 hover:bg-slate-50 hover:text-slate-900"}`}>
                                Civic Stories
                            </Link>
                        </div>
